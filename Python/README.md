...
* Used to quickly count things
* Can pass in any collection (I think) and it counts the things
* Special implementation of a `dict` where get `0` for `cntr['some_key']` if `'some_key'` not exists
* Very useful if need an unordered set that contains non-unique items (hence can't use `set`). E.g.,:
    ...
    closeStrings("aacabb", "ccbcaa") # True
    ```
...
* [notebooks/collate_covid19_daily_observation_summaries.py](notebooks/collate_covid19_daily_observation_summaries.py): regex, globbing, file I/O, type hints, list comprehensions.
