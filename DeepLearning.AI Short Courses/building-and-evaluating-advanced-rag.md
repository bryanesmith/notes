...
![Diagram of auto-merging retriever](images/building-and-evaluating-advanced-rag/auto-merging-retriever.png)

* Key components:
    - `HierarchicalNodeParser`
    - `Document` and their nodes (generated by above parser)
    - `ServiceContext`
    - `VectorStoreIndex` and `StorageContext`
    - `AutoMergingRetriever`
    - `RetrieverQueryEngine`, with (1) `AutoMergingRetriever` and (2) `SentenceTransformerRerank` (applied in post-processing step)

* Key variables:
    - `chunk_sizes` (e.g., `[2048,512]` for two layers, `[2048,512,128]` for three layers): used to specify size of chunks in hierarchy, starting with ancestor nodes and then descendant nodes
    - `similarity_top_k` (e.g., `12`)
    - `rerank_top_n` (e.g., `6`)
...
auto_merging_engine = get_automerging_query_engine(index, similarity_top_k=6)

response = auto_merging_engine.query(
...
* Sentence-window retrieval and auto-merging retrieval can significantly improve performance of RAG application
* Reducing LLM hallucination is going to be key to unlocking value of LLMs
* (1) Understanding data pipelines and (2) other evaluation metrics are good next steps